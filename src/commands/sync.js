...
import { getCurrentBranch, loadStacks, getCurrentRepo, removeBranchFromStack } from '../utils/stack.js';
import { fetchFromRemote, checkoutBranch, deleteBranch, getRemoteBranches } from '../utils/git.js';

const git = simpleGit();

export const syncCommand = new Command('sync')
  .description('Sync your local stack with remote changes and clean up stale branches')
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const behind = await getBehindCount(currentBranch, `origin/${currentBranch}`);
      
      if (behind > 0) {
        spinner.warn(chalk.yellow(`Current branch is ${behind} commits behind origin`));
      ...
      if (options.deleteMerged) {
        ...
        if (mergedBranches.length === 0) {
          ...
          mergedBranches.forEach(branch => console.log(chalk.dim(`  - ${branch}`)));
      ...
      for (const [stackName, stack] of Object.entries(repoStacks)) {
        for (const branch of stack.branches) {
          if (!remoteBranches.includes(`origin/${branch}`) && localBranches.all.includes(branch)) {
      ...
      if (staleBranches.length > 0) {
        ...
        staleBranches.forEach(branch => console.log(chalk.dim(`  - ${branch}`)));
...
async function getBehindCount(localBranch, remoteBranch) {
  try {
    const result = await git.raw(['rev-list', '--count', `${localBranch}..${remoteBranch}`]);
...
async function getMergedBranches(mainBranch) {
  try {
    ...
    const branches = result
      .split('\n')
      .map(b => b.trim())
      .filter(b => b && !b.startsWith('*') && b !== mainBranch && b !== 'master' && b !== 'main');
